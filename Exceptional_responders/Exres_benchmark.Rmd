---
title: "Analysis on structural variants present in the exceptional responders germline samples"
author: 
  #David Fuh, <br/>
  #Summer Intern, <br/>
...
```{r Import ExRes csv}
setwd("~/GitRepo/summerproject2021/Exceptional_responders")
# ExRes is a csv file
# that contains AnnotSV-annotated info on the structural variants
...
# Importing vcf files (Manta, SvABA, Wham only)
```{r Import ExRes vcf}
setwd("~/GitRepo/summerproject2021/Exceptional_responders/ExResVCF")
...
# Note that in the original SvABA output
# the sv type column needs to be replaced
# by the annotation done using the script
# "SvABA_BND_converter.R"

# After writing the sv type annotation in csv
# from the script, we import that info by
#
# svaba_vcf$SVTYPE = sample_ID.svtype.csv$SV_type


# load vcf
setwd("~/GitRepo/summerproject2021/Exceptional_responders/ExResVCF/Svaba_backup")
...
## SvABA SV type re-convert problem
...
Our previous caller benchmarking results using NA12878 WGS data shows:

> Manta outperforms other callers in recall rate at both 35X (79%) and even 18X (62%). However its precision is low (.36)
> Wham gave second best recall rate at 35X (57%), however with lower precision (.24)
> SvABA has good recall at 35X (56%) with excellent precision (.53)


Note that for each caller, their performance was measured/defined as:

- Recall = n(TP)/n(validated true variants)
- Precision = n(TP)/n(total call made by caller)


Based on the benchmarking results, we extract variants called by only 3 callers out of the 5 available based on their higher precision & recall performance at 35X (Note the mean WGS coverage for the cohort is around 30-40X), namely Manta, SvABA, and Wham.
...
```{r}
# we need a list of structural variant properties that we're interested in
# in terms of its biological relevance
# and other QC profiles present in the databases used by AnnotSV
...
SV_profile = c("SV_chrom", "SV_start", "SV_end", "SV_length", "SV_type", "ID", "REF", "ALT", "QUAL", "FILTER", "INFO", "FORMAT", "CytoBand", "Gene_name", "Frameshift", "Exon_count", "Location", "Location2", "ACMG_class",
               ...
               "ExResID", "Caller", "Ncallers") ## These variables were designed by us
```


## Immunologically related genes
```{r}
# 628 entries
# need citation and rationale of how this list was constructed
setwd("~/GitRepo/summerproject2021/Exceptional_responders/Variant_samples_csv")
...
First, we use vcf-generated genomic ranges objects (*.svgr) to locate the variants of interest
...
***Note: After confirming in GnomAD, the 55 bp insertion on intron2-intron2 of the gene MGMT in ER057 in fact is prevalent across the population (VAF=0.4517).***
...
# Joining main variant calls to one file

## main

```{r exres join}
full_join(ER019_q, ER108_q) %>% full_join(ER118_q) %>% full_join(ER095_q) %>% full_join(ER057_q) %>% full_join(ER052_q) -> ExRes_q
```

```{r Writing ExRes_q in RDS}
write_rds(ExRes_q, file="ExRes_q.rds")
```

```{r exres.special join}
# each ExRes variant calls
# that were deemed important by our workflow
# were stored in "ExResID.special" data frames
...
## Immunologically-related gene list
```{r megimmuno list}
# ER019
...
```{r N callers stat}
ExRes_q %>%
  ggplot() + aes(x=Ncallers) + geom_histogram()

table(ExRes_q$Ncallers)
```

```{r Variant call count by genome}
ExRes_q$SV_chrom=factor(ExRes_q$SV_chrom, levels = as.character(c(1:22, "X", "Y")))
ggplot(ExRes_q) + aes(x=SV_chrom, fill=ExResID) + geom_bar(position = "dodge") + theme_bw() + xlab("Chromosome") +ylab("Count")
ggsave("Variant_per_chr.png")
```

```{r SV type by caller}
variants %>% 
  ggplot() + aes(x=SV_type, fill=Caller) + geom_bar(position = "dodge")
```
...
```{r}
library(karyoploteR)
SV <- GRanges(seqnames=paste0("chr", ExRes_q$SV_chrom), 
              ranges=IRanges(start=ExRes_q$SV_start, end=ExRes_q$SV_end), 
              ACMG=ExRes_q$ACMG_class) 

kp <- plotKaryotype(genome="hg19")

SV_cool=SV[which(SV$ACMG %in% c("full=5", "full=4")), ]

kpPlotRegions(kp, SV, col="#FFAACC")

kpPlotRegions(kp, SV_cool, col="#CCFFAA")
```

```{r}
# ER019
SV019 <- GRanges(seqnames=paste0("chr", ER019_q$SV_chrom), 
              ranges=IRanges(start=ER019_q$SV_start, end=ER019_q$SV_end), 
              ACMG=ER019_q$ACMG_class) 

cool019=SV019[which(SV019$ACMG %in% c("full=5", "full=4")), ]

kp <- plotKaryotype(genome="hg19")

kpPlotRegions(kp, SV019, col="#FFAACC")

kpPlotRegions(kp, cool019, col="#CCFFAA")
```

```{r}
# ER057
SV057 <- GRanges(seqnames=paste0("chr", ER057_q$SV_chrom), 
              ranges=IRanges(start=ER057_q$SV_start, end=ER057_q$SV_end), 
              ACMG=ER057_q$ACMG_class)

cool057=SV057[which(SV057$ACMG %in% c("full=5", "full=4")), ]

kp <- plotKaryotype(genome="hg19")
kpPlotRegions(kp, SV057, col="#FFAACC")
kpPlotRegions(kp, cool057, col="#CCFFAA")
```


```{r}
# ER095
SV095 <- GRanges(seqnames=paste0("chr", ER095_q$SV_chrom), 
              ranges=IRanges(start=ER095_q$SV_start, end=ER095_q$SV_end), 
              ACMG=ER095_q$ACMG_class)

cool095=SV095[which(SV095$ACMG %in% c("full=5", "full=4")), ]

kp <- plotKaryotype(genome="hg19")
kpPlotRegions(kp, SV095, col="#FFAACC")
kpPlotRegions(kp, cool095, col="#CCFFAA")
```


```{r}
# ER108
SV108 <- GRanges(seqnames=paste0("chr", ER108_q$SV_chrom), 
              ranges=IRanges(start=ER108_q$SV_start, end=ER108_q$SV_end), 
              ACMG=ER108_q$ACMG_class)

cool108=SV108[which(SV108$ACMG %in% c("full=5", "full=4")), ]

kp <- plotKaryotype(genome="hg19")
kpPlotRegions(kp, SV108, col="#FFAACC")
kpPlotRegions(kp, cool108, col="#CCFFAA")
```


```{r}
# ER118
SV118 <- GRanges(seqnames=paste0("chr", ER118_q$SV_chrom), 
              ranges=IRanges(start=ER118_q$SV_start, end=ER118_q$SV_end), 
              ACMG=ER118_q$ACMG_class)

cool118=SV118[which(SV118$ACMG %in% c("full=5", "full=4")), ]

kp <- plotKaryotype(genome="hg19")
kpPlotRegions(kp, SV118, col="#FFAACC")
kpPlotRegions(kp, cool118, col="#CCFFAA")
```

```{r}
# ER052
SV052 <- GRanges(seqnames=paste0("chr", ER052_q$SV_chrom), 
              ranges=IRanges(start=ER052_q$SV_start, end=ER052_q$SV_end), 
              ACMG=ER052_q$ACMG_class)

cool052=SV052[which(SV052$ACMG %in% c("full=5", "full=4")), ]

kp <- plotKaryotype(genome="hg19")
kpPlotRegions(kp, SV052, col="#FFAACC")
kpPlotRegions(kp, cool052, col="#CCFFAA")
```
...
ExRes_q %>% filter(ACMG_class != "full=NA") %>%
