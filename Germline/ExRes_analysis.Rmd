...
cowplot::ggsave2("SV_count_chr_caller_exres.png")
```
...
cowplot::ggsave2("SV_count_dist_chr_ID_exres.png")
```
...
cowplot::ggsave2("SV_type_caller_exres.png")
...
ExRes %>% filter(Annotation_mode == "split") %>%
  ...
  ggplot(aes(x=Caller, y=SV_length, fill=Caller)) + 
  geom_boxplot() + 
...
cowplot::ggsave2("SV_size_exres.png")
```


### 3-2. SV length vs SV type 

```{r SV length in each type}
ExRes %>% filter(Annotation_mode == "split") %>%
...
cowplot::ggsave2("line_plot_SV_type_ExRes.png")
```


```{r SV length for two SV type categories, message=FALSE}
...
cowplot::ggsave2("CDS_SV_type_ID_exres.png")
...
```{r tile exon count & chromosomal location}
...
```{r tile Pathogenic variants sorted by Topology in ExRes genomes}
...
# alternative version (darker background)
CDS %>% filter(ACMG_class != "full=NA") %>%
  ggplot() + aes(x=SV_chrom, y=reorder(CytoBand, desc(CytoBand)), fill=ACMG_class) + 
...
```{r Mode of Inheritance}
# Mode of inheritance (moi)
CDS_variants_exres %>%
  ggplot() + aes(x=GenCC_moi) +
...
```{r Test plot: Two measures of pLI (GnomAD/ExAC), eval=FALSE}
# pLI: score indicating gene intolerance to a loss of function variation
ExRes %>% filter(Annotation_mode == "split") %>% .[sample(nrow(.), 200000), ] %>%
...
ggsave2("example_plot_exres_pli_scores.png")
```
