...
table(as.factor(maestro.small$FILTER))
```
...
ggsave("SV_length_vs_type_excluding_SNV_BND_NA12878.png")
```
...
```{r}
subset(maestro.small, select=c("ExAC_delZ",	"ExAC_dupZ",	"ExAC_cnvZ",	"ExAC_synZ",	"ExAC_misZ")) -> Z.mtx
```
...
##### Check up the complete Z score matrix (Z.mtx)
```{r}
Z.mtx %>%
  ggplot(aes(x=ExAC_delZ)) + geom_density()
```
