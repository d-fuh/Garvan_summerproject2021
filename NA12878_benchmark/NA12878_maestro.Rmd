...
ggsave2(filename = "SV_location_by_caller_NA12878_full_svaba.png")
```
...
ggsave2("SV_type_by_caller_NA12878_exc_svaba.png")
...
```{r message=FALSE}
# including SvABA
sample_size = benchmark.full %>% filter(Coverage == "Large") %>% group_by(SV_type) %>% summarize(num=n())

benchmark.full %>%
  filter(Coverage == "Large") %>%
  left_join(sample_size) %>%
  mutate(svtype = paste0(SV_type, "\n (n=", num, ")")) %>%
