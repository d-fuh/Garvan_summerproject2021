...
```{r}
# precious codes
MDS.xy$Caller = maestro.small[match(row.names(MDS.xy), row.names(maestro.small)),"Caller"]

MDS.xy$ACMG_class = maestro.small[match(row.names(MDS.xy), row.names(maestro.small)),"ACMG_class"]

MDS.xy$SV_type = maestro.small[match(row.names(MDS.xy), row.names(maestro.small)),"SV_type"]

MDS.xy$SV_length = maestro.small[match(row.names(MDS.xy), row.names(maestro.small)),"SV_length"]
```
...
ggplot(MDS.xy, aes(MDS1, MDS2)) + geom_point(aes(color = ACMG_class)) + theme_bw() +
...
ggplot(MDS.xy, aes(MDS1, MDS2, color = SV_type)) + geom_point() +
  ...
  theme(axis.title = element_blank(),
        panel.background = element_rect(fill = "grey17", colour = "black", size = 1),
        panel.grid.major = element_line(color = "grey30", size=0.1),
        panel.grid.minor = element_blank(),
        legend.title = element_text(size = 12),
        legend.key = element_rect(fill = NA)) +
