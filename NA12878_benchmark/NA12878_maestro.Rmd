...
> Shockingly --- over 10,000 entries had SV_length = 0. How could there be!?
...
large.full %>% filter(!is.na(SV_length)) %>%
  ...
  ylab("SV size") + xlab("") + ggtitle("SV size by callers in NA12878, excluding SvABA (l=1)")
...
filter(large.full, !is.na(SV_length)) %>% ## report these later
  ...
  ggplot(.) + aes(SV_length, fill=Caller) + geom_density() + scale_x_log10() + theme_classic() +
  xlab("SV size")
...
sizedSVs %>% filter(Caller == "Melt") %>%
  ...
  ggtitle("SV called by Melt, whose size is < 20 kb in NA12878") +
...
noNAlengthSV %>% mutate(SV_length = abs(SV_length)) %>% 
  ...
  scale_y_log10() + ylab("SV size") + xlab("Chromosome") + theme_few()
