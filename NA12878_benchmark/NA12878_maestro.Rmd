...
#MDS.xy$Topology = large.full[match(row.names(MDS.xy), row.names(large.split)),"Location2"]
```
...
ggsave2("MDS_ExAC_Z_by_ACMG_class_NA12878.png")
```
...
```{r}
# MDS: (4) By SV length
MDS.xy %>% mutate(SV_length = log1p(SV_length)) %>%
...
```{r}
# MDS: (5) By Gene topology
ggplot(MDS.xy, aes(MDS1, MDS2)) + geom_point(color = Topology, size = 1) +
  ...
  ggtitle("NMDS plot of five ExAC Z scores as measure of functional constraint \n grouped by gene topology from NA12878, n=600")

#ggsave2("MDS_ExAC_Z_by_topology_NA12878.png")
```
