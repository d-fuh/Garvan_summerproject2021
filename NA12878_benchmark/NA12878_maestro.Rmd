...
This is version 1.0 of the Maestro file. 
...
```{r}
maestro.small %>%
  filter(SV_length <= 20000) -> maestro.small
```
...
maestro.small %>%
  ...
  ggplot(aes(x=Caller, y=log10(SV_length), fill=Caller)) + geom_boxplot() + # best transform?
    theme_bw()
...
```{r}
subset(maestro.small, select=c("ExAC_delZ",	"ExAC_dupZ",	"ExAC_cnvZ",	"ExAC_synZ",	"ExAC_misZ")) -> MDS.mtx
```
...
```{r}
MDS.mtx[complete.cases(MDS.mtx), ] -> MDS.mtx ## removes all rows containing any NA
```


--> Apply similarity measure (Euclidean first?) --> S mtx via vegan::metaMDS
```{r eval=FALSE}
### CAUTION: FREEZES R
MDS.S <- metaMDS(MDS.mtx, distance = "euclidean", autotransform = FALSE, trace=FALSE)
```


##### Side-note: metaMDS runtime
```{r eval=FALSE}
# dim(MDS.mtx) = [7569, 5] for NA12878_full_annot file
start.time <- Sys.time()
...
```{r}
# profile fill in to MDS-coordinate frame
## How to ensure row-to-row match of profile?
MDS.xy$Caller <- maestro.small$Caller
MDS.xy$ACMG_class <- maestro.small$ACMG_class
MDS.xy$ACMG_class <- maestro.small$SV_type
MDS.xy$ACMG_class <- maestro.small$SV_length
```
...
```{r}
# MDS: (1) By Caller
ggplot(MDS.xy, aes(MDS1, MDS2, color = Caller)) + geom_point() +
...
ggsave("MDS_by_Caller_NA12878.png")

# MDS: (2) By ACMG class of SV
ggplot(MDS.xy, aes(MDS1, MDS2)) + geom_point(aes(color = ACMG_class)) +
...
ggsave("MDS_by_ACMG_class_NA12878.png")
```
```{r}
# MDS: (3) By SV length
ggplot(MDS.xy, aes(MDS1, MDS2)) + geom_point(aes(color = SV_length)) +
...
ggsave("MDS_by_SV_length_NA12878.png")

# MDS: (4) By SV type
ggplot(MDS.xy, aes(MDS1, MDS2, color = SV_type)) + geom_point() +
...
ggsave("MDS_by_SV_type_NA12878.png")
```
