...
Note that SvABA only counts BNDs in the entire call set, and has only SV_length = 1 (Even if we used strictly svaba.sv.vcf!)
...
> Q: Looks like we have some 0-sized SVs being called...how?
...
cowplot::ggsave2("SV_size_by_callers_NA12878_box.png")
```
...
filter(large.full, !is.na(SV_length)) %>% ## report these later
  ...
  mutate(SV_length = abs(SV_length)) %>% ## BE CAREFUL: log takes only X > 0 !! We are hence ignoring deletions here
  filter(., Caller %in% c("Delly", "Manta", "Wham")) %>%
  ggplot(.) + aes(SV_length, fill=Caller) + geom_density() + scale_x_log10() + theme_minimal() +
  xlab("SV size")

cowplot::ggsave2("Dentsity_SV_size_callers_exc_Melt_SvABA_NA12878.png")
```
...
sizedSVs %>% filter(Caller == "Melt") %>%
  ...
  ggtitle("Of those of |size| < 20 kb, SV length called by Melt in NA12878") +
...
#### 3-3. SV length by caller, under 20 kb, across chromosomes
```{r}
## original (unlimited) call set
p1 <- ggplot(noNAlengthSV, 
       aes(SV_chrom, SV_length, color=Caller)) + geom_point() + ylab("SV size")
## log
p2 <- noNAlengthSV %>% mutate(SV_length = abs(SV_length)) %>% 
  
  ggplot() + aes(SV_chrom, SV_length, color=Caller) + geom_point() +
  scale_y_log10() + ylab("SV size") + theme_cowplot()

p1
p2
ggsave2(p2, "SV_size_by_caller_across_chromosomes.png")
```
...
#### 3-4. ANOVA: SV length by caller, under 20 kb
```{r}
