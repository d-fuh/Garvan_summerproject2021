---
title: "Benchmarking Structural Variant Callers from NA12878 WGS Data"
edition: 2.0
date: "Jan 17, 2022"
author: 
  David Fuh, <br/>
...
# Filtering and processing notes:
    ...
    2) SV_length != 0 but NA is allowed. (?)
...
I am not sure about the VCF files yet (did not have time to go through them raw), but for the annotated TSVs I'm working with, I've noticed: 
    ...
    2. the SvABA VCF has undergone import processing (see ~/SvABA_processing)
    3. the Melt calls were much smaller (-500 ~ 6,500 bp) than Delly, Manta, Wham (mean SV_length ~ 3mb)
...
> The read count is used as a proxy for quality when quality score is not reported. Some callers report all calls and stratify them into pass (which can be considered as high confidence) and non-pass (low confidence), while others report only pass calls.^[Cameron, D. L., et al. (2019). "Comprehensive evaluation and characterisation of short read general-purpose structural variant calling software." Nature Communications 10(1): 3240.]
...
> Q: Explain the recordings in Y chromosome.
...
We see that the pink column is the true graph we wanted in Section 1-1.
...
```{r}
# including SvABA
benchmark.full %>% filter(Coverage == "Large") %>%

ggplot(aes(x = SV_chrom, fill=Caller)) +
    geom_bar() +
    theme_bw() +
    labs(x="Chromosome", y="count") + scale_fill_brewer(palette = "Set2") +
    ggtitle("SV location called by the five callers used on NA12878 (AnnotSV)")

ggsave2(filename = "SV_location_by_caller_NA12878.png")
```
...
ggsave2("SV_type_by_caller_NA12878.png")
...
Note that:
    ...
    2. [SvABA] only counts BNDs in the entire call set, and has only SV_length = 1. This was solved by annotation.



#### 2-2. SV types vs coverage level (Small, Medium, Large subset reads of NA12878)
...
# Bar plot
  ggplot(aes(SV_type, mean.A, fill=Coverage)) +
    ...
    geom_text(aes(label = paste(round(mean.A, 0))), position = position_dodge(0.9), 
...
> Q: **duplication due to repeated calls?**
...
- Note that SV size = 1 for all svaba.sv calls.
...
```{r}
hist(log1p(large.full$Gene_count))
```
