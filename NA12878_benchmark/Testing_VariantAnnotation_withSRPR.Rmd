...
```{r create delly.bpr and delly.bpr.SRPE (Both svgr objects)}
delly.bpr.SRPE=breakpointRanges(delly_vcf, info_columns=c("SR", "PE", "RC"))
#delly.bpr=breakpointRanges(delly_vcf)
#head(delly.bpr) ## without SR & PE columns
head(delly.bpr.SRPE) ## with SR & PE columns, multi NA
```


### Assign a column for caller name
```{r Assign $Caller Delly}
delly.bpr.SRPE$Caller="Delly"
```


### Need to filter out the variants that pass
```{r Delly filter}
delly.bpr.SRPE=delly.bpr.SRPE[which(delly.bpr.SRPE$FILTER=="PASS")]
```
...
### Need to filter out the variants that pass
```{r}
svaba.bpr.SRDR=svaba.bpr.SRDR[which(svaba.bpr.SRDR$FILTER=="PASS")]
```
...
```{r augment svaba svgr with the new SR/DR info stored in its vcf, warning=FALSE}
# there is a warning (a very long one) regarding chrM
svaba.bpr.SRDR=breakpointRanges(svaba_vcf) ## update svgr
svaba.bpr.SRDR$Caller="SvABA" ## assign caller
```


```{r}
# count bp overlaps between delly and svaba
delly.bpr.SRPE$svabamatch=countBreakpointOverlaps(delly.bpr.SRPE, svaba.bpr.SRDR, 
                                              maxgap=100, sizemargin = 0.25, 
                                              restrictMarginToSizeMultiple = 0.5, countOnlyBest=T)

head(delly.bpr.SRPE) ## check updated svgr

table(delly.bpr.SRPE$svabamatch) ## see how many calls from delly matched with svaba
```


## find overlaps between svaba and truth
```{r}
truth_personalis.bpr$svabamatch=countBreakpointOverlaps(truth_personalis.bpr, svaba.bpr.SRDR, 
                                                        maxgap=100, sizemargin = 0.25,
                                                        restrictMarginToSizeMultiple = 0.5, countOnlyBest=T)
...
```{r}
delly.bpr.SRPE$truthmatch=countBreakpointOverlaps(delly.bpr.SRPE, truth_personalis.bpr, 
                                                  maxgap=100, sizemargin = 0.25, 
                                                  restrictMarginToSizeMultiple = 0.5, countOnlyBest=T)
head(delly.bpr.SRPE)
table(delly.bpr.SRPE$truthmatch)
```
...
```{r}
summary(delly.bpr.SRPE$SR[which(delly.bpr.SRPE$truthmatch==1)])
boxplot(delly.bpr.SRPE$SR[which(delly.bpr.SRPE$truthmatch==1)])
```

Similarly, for PE:
```{r}
summary(delly.bpr.SRPE$PE[which(delly.bpr.SRPE$truthmatch==1)])
boxplot(delly.bpr.SRPE$PE[which(delly.bpr.SRPE$truthmatch==1)])
```
